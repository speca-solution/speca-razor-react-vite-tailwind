...
| — | **Batas jujur E10-05:** bundle id iOS tetap `com.speca.*` di instance (pbxproj copyOnly demi `containerPortal`) — rename manual via Xcode saat setup signing di Mac; `--data-comm none` + mobile: demo `main.dart` tetap ter-build (gen Dart standalone) tapi SayHello gagal runtime karena server tanpa gRPC; script `desktop` di package.json root tersisa saat platform tanpa desktop (JSON tak bisa dikondisikan — preseden E9-06). | tercatat |
| E10-08 ☑ | **Pilihan platform tunggal** `--platforms web\|desktop\|mobile\|all` (alias `-p`, default `all`) menggantikan dua flag bool `--desktop`/`--mobile`; keduanya jadi simbol **computed** dari pilihan ini (web selalu ada = fondasi). Deskripsi tiap pilihan menyebut prasyaratnya (web: tanpa Rust/Flutter; desktop: Rust; mobile: Flutter+Android SDK) | terverifikasi keempat pilihan: `web`→Desktop✗ Mobile✗ bufDart✗ pubScript✗; `desktop`→Desktop✓ pubScript✓ Mobile✗; `mobile`→Mobile✓ bufDart✓ Desktop✗; `all`→semua ✓. CI job template diperbarui ke `--platforms web` |
...
| E11-04 ☑ | **gRPC enforcement**: `StreamTicks` `[Authorize(BearerOnly)]` (#if useAuth) — tanpa token nol tick bocor; `SayHello` tetap anonim tapi baca identitas tervalidasi (sub) bila token valid; `rpc-smoke.ts` ADAPTIF (login demo→token nyata bila /auth ada, anonim bila tidak — valid di kedua instance tanpa conditional) | terverifikasi CI runner (commit 3fa7da0): source useAuth = token→4 tick + tanpa-token ditolak (0 bocor); instance `--auth none` = anonim 4 tick. Catatan jujur: gRPC-Web petakan 401 `[Authorize]` → kode `Internal` (bukan `Unauthenticated`); asersi menjamin NOL data bocor, bukan kode |
| E11-05a ☑ | **Klien mobile (Flutter)**: `flutter_secure_storage` (Android Keystore AES-GCM / iOS Keychain) simpan access+refresh; `AuthApi` (HTTP `/auth/login\|refresh\|logout`, badCert dev-only guard kDebugMode); `GreeterClient` lampirkan Bearer di metadata gRPC + **auto-refresh sekali** saat ditolak; UI login→StreamTicks ber-auth→logout | terverifikasi emulator (screenshot): login demo→JWT di Keystore→StreamTicks `[Authorize]` diterima→"4 tick ber-auth" (Tick 1,2,3,4); server log `gRPC stream StreamTicks 1259ms`; `flutter analyze` bersih + widget test hijau |
| E10-07 ☑ | **Design system + UI kit mobile** (Flutter tak baca CSS → token web disalin ke Dart): `theme/speca_tokens.dart` (warna+bentuk theme1 #1b84ff & theme2 #7367f0, light+dark, ThemeExtension) + `theme/speca_theme.dart` (`buildSpecaTheme()` → ThemeData, widget Material otomatis bergaya web; varian via `--dart-define=SPECA_THEME`); `ui/` kit GENERIK: SpecaStatusChip/ChoiceChips/QtyStepper/Segmented/StatusTimeline/SummaryList/StatCard/MenuTile/SectionHeader/HighlightCard; `screens/ui_kit_demo.dart` galeri + bottom nav | terverifikasi emulator (2 screenshot): semua komponen render sesuai token theme1; `flutter analyze` bersih + widget test hijau. **Batas jujur:** token disalin MANUAL — bila `_tokens.css` berubah, `speca_tokens.dart` harus ikut diperbarui (belum ada generator/sinkronisasi otomatis) |
| E11-05b ☐ | **Klien web (TS/React)** & **desktop (Tauri, reuse TS)**: fetch `/auth`, access token di memori (web pakai cookie utk Razor; token API utk panggilan gRPC-Web ber-auth); interceptor Connect-ES lampirkan Bearer + auto-refresh | login→token→gRPC-Web ber-auth dari RpcDemo |
