...
| E10-06 ◐ | **CI matrix multiplatform hemat kuota** (repo private, Windows 2×): `ci.yml` + `concurrency cancel-in-progress` + uji template diperluas (grep speca Desktop/Mobile/scripts, `gradlePluginPortal()` utuh, `.gitkeep`, varian `--desktop false --mobile false`); `desktop.yml` = `cargo check --locked` di windows-latest, **path-filtered** (`Apps/Desktop/**`) + rust-cache; `mobile.yml` = analyze+test+APK debug di ubuntu, **path-filtered** (`Apps/Mobile/**`, proto, buf.gen.yaml) + cache flutter/gradle; `platform-weekly.yml` (Sabtu 21:00 UTC + manual) = tauri build penuh → artifact installer NSIS (retensi 14 hari) + APK dari instance template | SISA VERIFIKASI: push ke GitHub → keempat workflow hijau pada run pertama (tak bisa diuji lokal) |
