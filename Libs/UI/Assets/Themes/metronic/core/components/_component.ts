...
import KTGlobalComponentsConfig from './_config';
import { KTBreakpointType, KTOptionType } from '../_types';
		...
		if (parts.length > 1) {
			for (let i = parts.length - 1; i < parts.length; i--) {
				...
				if (part.includes(':')) {
					...
					const breakpoint = KTUtils.getBreakpoint(
						breakpointKey as KTBreakpointType
					);
