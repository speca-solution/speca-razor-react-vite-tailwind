...
import KTData from '../../helpers/_data';
import KTEventHandler from '../../helpers/_event-handler';
import KTComponent from '../_component';
import { KTMenuConfigInterface, KTMenuInterface } from './_types';
import { KTMenuItemToggleType, KTMenuItemTriggerType } from './_types';
import { KTOptionType } from '../../_types';
import { KT_ACCESSIBILITY_KEYS } from '../_constants';
declare global {
	interface Window {
...
export class KTMenu extends KTComponent implements KTMenuInterface {
	...
	protected _mouseout(element: HTMLElement): void {
		...
		if (relatedTarget) {
			const childItemElement = this._getItemElement(relatedTarget);
	...
	protected _isChildOfParent(
		...
		const childSubElement = this._getItemSubElement(childElement);
		if (childSubElement && parentElement.contains(childSubElement)) {
			return true;
		}

		// Check if child is within parent's dropdown
		const parentSubElement = this._getItemSubElement(parentElement);
	...
	protected _hideUnrelatedDropdowns(currentItemElement: HTMLElement): void {
		...
		const preserveParents = this._getOption('preserveParentDropdowns') as boolean;
		...
		itemElements.forEach((itemElement) => {
			...
			if (
				...
				if (preserveParents && this._isChildOfParent(currentItemElement, itemElement as HTMLElement)) {
					return;
				}

				// Don't hide if current element is a child of this element
				if (this._isChildOfParent(itemElement as HTMLElement, currentItemElement)) {
					return;
				}
	 ...
	 */
	public static cleanup(skipElement?: HTMLElement): void {
		const elements = document.querySelectorAll('[data-kt-menu]');
		elements.forEach((element) => {
	...
	public destroy(): void {
		...
		if (KTData.has(this._element, 'popper')) {
			const popper = KTData.get(this._element, 'popper') as PopperInstance;
		...
		const subElements = this._element.querySelectorAll('.kt-menu-dropdown, .kt-menu-accordion');
		subElements.forEach((subElement) => {
			KTData.remove(subElement as HTMLElement, 'menu');
			KTData.remove(subElement as HTMLElement, 'item');
		});

		// Remove overflow elements from body
		const overflowElements = document.querySelectorAll('[data-kt-menu-sub-overflow="true"]');
